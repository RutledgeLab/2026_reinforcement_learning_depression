function [LL, model_var] = model_hbi_RL_outcome_initSmall_distSmall_combined(param, data, idx_free, param_default, isSim)
...
for b = 1:nBlock
    ...
    % outcome
    win_loss = data{b}.win_loss;
    list_prob = data{b}.list_prob;
    ...
    if sum(trial_subject_choice(:))==0 & isSim==0
    ...
    for t = 1:nTrial_total
        ...
        if sum(trial_subject_choice(t,:))==0 & isSim==0
            break
        end
        ...
        if isSim==1
            ...
            % generate and save objective outcome
            current_choice = find(trial_model_choice(t,:)==1);
            outcome_rand = rand(1);
            if outcome_rand<list_prob(idx_choice)
                data{b}.trial_amt_outcome(t, idx_choice) = large_reward;
                outcome = large_reward; % large sv
            else
                data{b}.trial_amt_outcome(t, idx_choice) = small_reward;
                outcome = small_sv; % small sv
            end
